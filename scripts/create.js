...
function displayInput(input) {
  ...
  <div class="all-card">    
    <div class="card">
          <h1>${input.title}</h1>
          <p>${input.date}</p>
          <p>${input.content.slice(0, 100)}${input.content.length > 100 ? "..." : ""}</p>
          <div class="button-arrow">
            <a href="#" onclick="deleteInput('${input.id}')"><button>Delete</button></a>
            <a class="arrow-right" href="../pages/viewpage.html?id=${input.id}&title=${input.title}&date=${input.date}&content=${input.content}">
